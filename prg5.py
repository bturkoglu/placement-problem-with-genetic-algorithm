...
class Cisim:
	...
	def yerlestir(self, x, y):
		...
		self.x1 = x + delta
		self.y1 = y + delta
		self.x2 = self.x1 + self.taban - 2*delta
		self.y2 = self.y1 + self.yuks - 2*delta
		self.yerlesti = 1
...
class Zincir:
	def __init__(self, dnalar, siralar, yonler, zeminyuksekligi):
		self.zeminyuksekligi = zeminyuksekligi
...
class Hesap:
	...
	def yenibireyCaprazlayarakYeni(self, bireyno1, bireyno2):
		# Tam caprazlamak.Once 1.nin 1.dnasi, sonra 2.nin 1.dnasi
		# alınarak, 2n boyunda dna uretilir. Sonra içinden cift olanlar atilir
		b1sira, b1yon, fit1 = self.bireyler[bireyno1]
		b2sira, b2yon, fit2 = self.bireyler[bireyno2]

		yenisira = []
		yeniyon = []
		for i in range(self.dnaAdet):
			yenisira.append(b1sira[i])
			yenisira.append(b2sira[i])
			yeniyon.append(b1yon[i])
			yeniyon.append(b2yon[i])

		yenibireysira = []
		for i in yenisira:
			if i not in yenibireysira:
				yenibireysira.append(i)

		yenibireyyon = yeniyon[:self.dnaAdet]

		yenisira =tuple(yenibireysira)
		yeniyon = tuple(yenibireyyon)

		#Mutasyon yapilacak mi?
		if random.randint(1,100) <= self.mutasyon_yuzdesi:
			self.yapilanmutasyonsayisi += 1
	...
	def yenibireyCaprazlayarak(self, bireyno1, bireyno2):
		...
		parca1 = b1sira[:bolumyeri]
		parca2 = [n for n in b2sira if n not in parca1]
		yenisira =tuple(parca1) + tuple(parca2)
	...
	def bireyHesapla(self, bireyno):
		...
		zincir = Zincir(self.dnalar, bireysira, bireyyon, self.zeminyuksekligi)
	...
	def nesliHesapla(self):
		...
		print('Fitness:',list(self.sonuclar.values()))
	...
	def hesabaBasla(self):
		...
		print('Mutasyon Sayısı:', self.yapilanmutasyonsayisi)
